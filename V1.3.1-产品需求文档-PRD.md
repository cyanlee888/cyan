...
- **体验课一次性（消耗即锁，不可重上）**：任一 level 的体验课是**全局一次性资源**，上过即计入「已消耗」。已消耗的 level 不再出现 `Try this level` / 降级横幅 `Try L{n}` 等「再上体验课」入口，统一替换为 `View report`——回看的报告**恒等于当前定级**（学习计划页也恒显示 current）。由此保证：不存在「同一档体验课反复重上、反复重定级」，也不存在「看旧 level 报告却给出另一档计划」（例如模糊定级 L2→上 L2 体验课定级 L3 后，L2 已消耗，只能回看结论为 L3 的报告，不能再上 L2）。
- **升 / 降统一入口 = 当前 level 顶部「双向 stepper」（升降对称，一次一档）**：跨 level 的升 / 降**不再散落在各档卡片 / 单元上**，而是统一收敛为当前 level Learning Path 顶部的一个双向 stepper，升降形态完全一致：
  - **↑ 升级（current+1）**：`Ready for the next level?` + `Try {L+1}`——免费=定级试课（±1 真实定级 → 学习计划 → Paywall）；会员=升要证明（达标才升、未达标保持、**永不降级**）。current+1 体验课已消耗 → 该行变 `View report`（回看=current）。
  - **↓ 降级（current-1）**：`Feeling too hard?` + 会员 `Switch to {L-1}`（直接切、不上体验课）/ 免费 `Try {L-1}`（易档定级试课）。出现条件：current == 定级起点（未自然进阶）；免费还需该更低档体验课未消耗。
  - **一次一档**：升 / 降都只作用于相邻 ±1；需要跨多档时逐档 step（切到 L-1 后，其顶部 stepper 再显示 ↓ L-2）。各档卡片一律不再出现 `Try` / `Switch`，越界档统一显示 `Locked` 并引导回顶部 stepper。
  - 设计取舍：升降都用横幅式 stepper 而非卡内按钮——既保证**升降交互一致**，又让「太难」这种被动求助场景有主动兜底；升级方向有明确条件、非常驻怂恿，与「按 level 循序渐进」不冲突。
- **跨 level 课程访问口径（浏览 vs 学习分离）**：排课只锚定 current level；其它 level 的 Lesson 内容（words / sentences / objectives）**任意 level 都可只读浏览**，但「真正开始上」按身份与 level 关系判定，避免出现「显示 Start Class 却进不去」的死胡同。
  - **升 / 降只走顶部双向 stepper**（见上），卡片本身**不再承载任何跨 level 的 `Try` / `Switch`**；卡片只表达「当前 level 学习 / 复习 / 只读浏览 / 锁定」四种状态。
  - **免费用户**（正式 Lesson 都上不了，体验课是唯一产品体验），Lesson 卡片 CTA 按 level 关系：
    - **当前 level**：所有正式 Lesson（Lesson 1 起）一律 `Go Premium` → Paywall。免费用户唯一能免费上的是**体验课**（onboarding 首测 / 顶部 stepper 试更高 / 试更低），体验课是独立入口，不属于正式课序列。
    - **其它任意 level 卡片**（current+1 / current-1 / 更高 / 更低）→ 一律 `Go Premium`；升 / 降改由顶部 stepper 发起（本期试课范围只到相邻 ±1）。
  ...
  - **会员用户**（拥有全部内容），Lesson 卡片 CTA 按 level 与进度关系：
    ...
    - **越界档卡片**（current+1、被跳过的更低档、> current+1 更高档）→ 一律 `Locked`，点击提示「用当前 level 顶部的 ↑ / ↓ 切换」；升 / 降统一走顶部 stepper（一次一档，逐档 step）。
  - **一次一档**：升 / 降均只作用于相邻 ±1（首体验课 ±1、升级只判断 current→current+1、降级只到 current-1）；跨多档逐档 step。
