...
- **体验课定级权责（统一口径：升要证明 / 降靠自己 / 初始靠首测）**：一条原则——**体验课只在「用户当前要去的方向」上做一次判定，且永不越过一个「已坐实的定位」把人往下拉**。据此三条线各管一段、互不矛盾：
  - **初始定位（首次体验课）**：此时没有已坐实的定位，模糊定级只是「上哪节体验课」的猜测；体验课**从零建立**真实定位，落在所上 level **±1**（看起来可上可下，本质是对一个未坐实猜测的校准，非升降）。这是**唯一凭体验课定级**的场景。
  - **升级尝试（Try this level，current+1）**：current 已被首测 / 学习验证。体验课只回答「够不够上 current+1」——**达标=升 current+1 / 未达标=保持 current**，**永不降级**。一节更难的课不构成否定既有 current 的证据。
  - **降级（current-1）**：由用户**主动发起**、表达「当前太难」，**不重新定级**（会员直接切排课、免费走体验课转化）。系统不替用户做降级决定。
  - 结论：**没有任何「上了一节体验课就被系统意外降级」的路径**；真正的下行只来自用户自己点降级。
- **跨 level 课程访问口径（浏览 vs 学习分离）**：排课只锚定 current level；其它 level 的 Lesson 内容（words / sentences / objectives）**任意 level 都可只读浏览**，但「真正开始上」按身份与 level 关系判定，避免出现「显示 Start Class 却进不去」的死胡同。
  - **免费用户**：除自己那唯一一节体验课外，**任何 level 的任何正式 Lesson 一律 `Go Premium` → Paywall**；不存在「学低 level」一说，低 level 仅作为 LP 降级横幅的转化漏斗（试更简单体验课 → Paywall）。
...
| 核心能力 | • 竖屏单栏 Paywall<br>• 本周批次上完探索引导<br>• 登录设备上限 3 台 + 自动登出<br>• 升降级定级安全阀（不对称口径：升要证明 / 降不要）<br>• 背包手办 IP / 系列分层<br>• Kakao 登录后手机号缺失强制绑定 |
...
flowchart TB
  ...
  Confirm -->|Level up · 会员| UpdatePlanM[切排课到 current+1<br/>跳级]
  Confirm -->|Level up · 免费| PaywallUp[Paywall<br/>trigger_source=level_up]
...
| 升降级 | 升降级总则与口径 | 无独立 UI（口径贯穿下列各行） | • **统一原则**：体验课只在「用户当前要去的方向」上判定一次，**永不越过已坐实的定位把人往下拉**——升要证明 / 降靠自己 / 初始靠首测<br>• 口径：<br>　- 首次体验课（初始定位）：报告按所上 level **±1** 产出真实定级、直接采用排课（唯一凭体验课定级处）<br>　- 试更高 current+1（升级尝试）：免费 / 会员都上体验课，**达标=升 current+1 / 未达标=保持 current，永不降级**；达标后 `Level up my plan`，用户手动确认<br>　- 降级 current-1（独立入口）：用户主动发起、**不重新定级**；免费上体验课转化（→ 右档报告 → Paywall）；会员**不上体验课**，正式课直接切到 current-1<br>• 设计约束：**没有任何「上一节体验课就被系统降级」的路径**；**不做自动改级**（不来回横跳），所有改级均由用户手动确认 | 接口：当前定级 / 排课更新（沿用 V1.3.0 + 本期扩展） | P0 |
...
| Level·Unit 列表（Learning Path） | 跨 level 课程 Lesson CTA | • Lesson 卡片按「身份 × level 关系」给出唯一 CTA，不再出现误导性 `Start Class` | • **免费**：非当前体验课的任何 Lesson → `Go Premium`（→ Paywall）<br>• **会员**：已学过低 level → `View report`（只读，不可重上）；被跳过低 level → `Switch to L{n}`（二次确认 → 直接重锚排课到该 level Unit1，不上体验课 / 不重新定级，可一步切到任意更低被跳过 level）；current+1 → `Try this level`（定级复核）；更高（>current+1）→ `Locked`<br>• 所有 level 的 Lesson 内容（words / sentences / objectives）均可**只读浏览** | 接口：当前定级 / 排课更新（沿用 V1.3.0） | P1 |
...
| 升级尝试（Level check） | 试更高 Level · 达标则升、永不降 · 手动确认 | • current+1 首个 Unit 的入口，按钮 `Try this level`（尝试这个 Level 的课程）<br>• 入口下提示：试这一节课，看看是否准备好升到下一档<br>• 报告页展示判定结果（CEFR + Level）+ 行动按钮 | • 出现场景：current+1 首个 Unit<br>• 点入口 → 上 current+1 体验课 → **只判定「够不够上 current+1」**：<br>　- **达标** → 标题「ready to level up」，并列 `Level up my plan` + `Keep my current plan`，用户手动确认<br>　- **未达标** → 标题「Not quite ready for {target} yet」，**仅** `Keep my current plan`，**保持 current、永不降级**<br>• `Level up my plan`：会员切排课到 current+1（跳级）；免费 → Paywall（`trigger_source=level_up`）<br>• **本入口不产生降级**（一节更难的课不否定已坐实的 current）；想降级走 current-1 降级入口<br>• 体验课不可重上则直接看已有报告；保持现状不消耗入口 | 接口：沿用 V1.3.0；判定返回 current 或 current+1 | P1 |
