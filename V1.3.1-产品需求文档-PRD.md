...
6. **卡号登录手机号绑定**：使用卡号登录成功后，若未获取到用户手机号，强制进入手机号绑定页（输入手机号 + 短信验证码）完成绑定后再进入 App。
...
| 卡号登录 | 用学习卡的卡号 + 卡密登录账号的方式；卡号账号可能未携带手机号。 |
...
- **背景与判断**：
  - 主链路为「onboarding 自评（模糊定级）→ 上对应 level 体验课 → 体验课报告产出正式定级（±1 微调）→ 排课」。模糊定级是家长自评，前期为快速进入产品常常随手选，**天然不可靠**；体验课正式定级也只能在 current±1 内评估、一次最多 ±1。
...
- **背景**：卡号（学习卡）登录的账号常常只有卡号 / 卡密、不含手机号；而手机号是后续找回账号、客服触达、营销触达与账号安全（验证身份）的关键联系方式。缺手机号会导致后续服务与触达断档。
- **目标**：在卡号登录成功后，先判断账号是否已获取到手机号；**未获取到则强制进入手机号绑定页与流程**（手机号 + 短信验证码校验），绑定成功后再进入 App；已获取到则直接进入，不打扰。
...
| 本期页面 | Paywall（竖屏改版）、Class 首页（探索引导卡）、账号 / 登录（设备上限提示、卡号登录手机号绑定页）、Level·Unit 列表（降级入口）、体验课链路（免费下探入口）、升级 Level up、背包 · 手办 Tab（IP / 系列分层）。 |
| 核心能力 | • 竖屏单栏 Paywall<br>• 本周批次上完探索引导<br>• 登录设备上限 3 台 + 自动登出<br>• 升降级定级安全阀（不对称口径：升要证明 / 降不要）<br>• 背包手办 IP / 系列分层<br>• 卡号登录后手机号缺失强制绑定 |

---

## 六、关键流程

### 6.1 升降级总流程（不对称：升要证明 / 降不要）
...
flowchart TB
  ...
  UpTrialM --> UpReport[定级报告<br/>current±1 真实定级]
  UpTrialF --> UpReport
  UpReport --> Cmp{定级 vs 当前 Level}
  Cmp -->|相等| Keep[Keep my current plan<br/>保持当前 Level]
  Cmp -->|更高 / 更低| Confirm{用户手动确认 Update?}
  Confirm -->|Keep my current plan| Keep
  Confirm -->|Update · 会员| UpdatePlanM[按定级切排课<br/>更高=跳级 / 更低=降级]
  Confirm -->|Update · 免费| PaywallUp[Paywall<br/>更高 level_up / 更低 downgrade]
  ...
  DownTrialF --> DownReport[体验课报告（右档）]
...
| 账号 / 登录 | 卡号登录手机号绑定 | • 登录方式入口新增「卡号登录」：卡号 + 卡密 → `Log in`<br>• 手机号绑定页（竖屏 takeover，参考 Figma 4867:18878）：<br>　- 标题 `Please add a contact phone number` + 副文案<br>　- 已登录提示行：`Your card account has been logged in`<br>　- 手机号输入：区号选择（默认 +966）+ 手机号<br>　- 短信验证码：输入框 + `Get code`（发送后倒计时）<br>　- 底部 `Log in` + 说明 `A verification code will be sent to your phone via SMS` | • 流程：选择卡号登录 → 卡号 / 卡密校验通过 → **判断账号是否已获取到手机号**<br>　- 已获取 → 直接进入 App，不展示绑定页<br>　- 未获取 → 进入手机号绑定页（**强制步骤**，绑定成功前不进入 App）<br>• 绑定页：输入手机号后 `Get code` 可点 → 发送短信验证码并倒计时；输入手机号 + 验证码后 `Log in` 可点 → 校验验证码<br>　- 校验成功 → 手机号绑定到账号 → 进入 App<br>　- 校验失败 → 留页提示重试<br>• 边界：手机号绑定为强制步骤，无跳过；区号支持多国；验证码有发送频控（60s 倒计时再发） | 接口：卡号登录（卡号 / 卡密校验，返回是否含手机号）；发送短信验证码；手机号 + 验证码绑定（新增）；字段 `phone`、`country_code`、`phone_bound` | P1 |
| 升降级 | 升降级总则与口径 | 无独立 UI（口径贯穿下列各行） | • **不对称总则**：升级 = 往难处走，需要证明（必须上体验课 / 测当门槛）；降级 = 往易处走，不需要证明<br>• 口径（两原则交叉而成）：<br>　- 试更高 current+1：免费 / 会员都上体验课，报告按 **current±1 真实定级**更新——定级更高则跳级、等于当前则保持、更低则降级；升 / 降统一 `Update my plan`，用户手动确认<br>　- 降级 current-1（独立入口）：免费上体验课（→ 右档报告 → Paywall）；会员**不上体验课**，正式课直接切到 current-1<br>• 设计约束：**一次一档**（评估限 current±1）；**不做自动降级**（不把 level 自动改动、不来回横跳），所有改级均由用户手动确认 | 接口：当前定级 / 排课更新（沿用 V1.3.0 + 本期扩展） | P0 |
| Level·Unit 列表（Learning Path） | 降级入口（current-1 横幅） | • 出现场景下于单元列表顶部展示 `Feeling too hard?` 横幅<br>• 行动按钮按身份：会员 `Switch to {Level}` / 免费 `Try {Level}` | • 出现场景：所选 level == current-1 时出现；其他 level 不展示<br>• 会员点击 → 确认后**直接切排课到 current-1（不上体验课）** + toast；当前 Level 与定级起点同步下移到 current-1<br>• 免费点击 → 上 current-1 体验课 → 右档报告 → 学习计划领取页 → Paywall（`trigger_source=downgrade`）<br>• 边界：切换不可逆需二次确认 | 接口：当前定级 / 排课更新 | P0 |
...
| 升级 / 定级复核（Level check） | 试更高 Level · 按真实定级更新（可升可降 · 手动确认） | • current+1 首个 Unit 的入口，按钮 `Try this level`（尝试这个 Level 的课程）<br>• 入口下提示：试这一节课，我们会据此给出最合适的计划<br>• 报告页展示真实定级（CEFR + Level）+ 行动按钮 | • 出现场景：current+1 首个 Unit<br>• 点入口 → 上 current+1 体验课 → 报告产出**真实定级**（评估范围 current±1，可能为 current+1 / current / current-1）<br>• **按「定级是否等于当前 Level」决定（不用达标 / 未达标）**：<br>　- 定级 == 当前 Level → 仅 `Keep my current plan`<br>　- 定级 ≠ 当前 Level（更高或更低）→ 并列 `Update my plan` + `Keep my current plan`，**升 / 降统一文案、不区分**，由用户手动确认<br>• `Update my plan`：会员按定级切排课（更高 = 跳级 / 更低 = 降级，定级起点随更低同步下移）；免费 → Paywall（更高 `trigger_source=level_up` / 更低 `trigger_source=downgrade`）<br>• 体验课不可重上则直接看已有报告；保持现状不消耗入口 | 接口：沿用 V1.3.0；定级返回 current±1 内真实 level | P1 |
| 背包 · 手办 Tab | 手办 IP / 系列分层 | • 顶部 IP 子 Tab 行：IP emoji + 名称 + 已获得计数；**仅展示「含至少 1 个已获得手办」的 IP**，按固定顺序 Dino → Mama → Papa → Baby…<br>• 选中 IP 下按**系列名分组**：系列标题 + 该系列已获得数 + 该系列已获得手办卡片（沿用 ×N 重复计数）；仅展示该 IP 下含已获得手办的系列<br>• 空态：无任何已获得手办时展示原空态（去玩盲盒） | • 出现场景：背包切到「手办」Tab<br>• 默认选中第一个有已获得手办的 IP<br>• 点 IP 子 Tab → 切换并重渲染该 IP 的系列分组<br>• 设计约束：手办仍为**纯收集**，仅展示已获得，无选中 / 无佩戴、无未获得占位 / 锁定态 | 数据：手办新增 `ip`、`series` 字段；IP / 系列配置（label、emoji、order）；用户已获得手办及计数（沿用 V1.3.0） | P1 |
...
| --- | --- | --- | --- | --- | --- |
| V1.3.1 | 2026-06-28 | 手机号绑定页 | `phone_bind` | 卡号登录后未获取到手机号，进入绑定页曝光 | `login_method`：card |
...
| V1.3.1 | 2026-06-28 | 背包手办 IP 子 Tab | `backpack_figure_ip_tab` | 在手办 Tab 切换 IP 子 Tab | `ip`（dino、mama、papa、baby…） |
| V1.3.1 | 2026-06-28 | 登录页卡号入口 | `login_card_entry` | 点击「卡号登录」入口 | 无额外参数 |
...
| V1.3.1 | 2026-06-28 | 卡号登录 | `card_login_result` | 卡号 + 卡密登录返回结果 | `result`：success、fail；`has_phone`：true、false（success 时）；`description`：card_invalid、card_used（fail 时） |
...
- 卡号登录单独建 `card_login_result`、不扩展受约束的 `login_result.method`（仍 apple、google）；`login_method`（card）作为 `phone_bind` 曝光的低基数参数；`has_phone`（true、false）标识是否需绑定；失败短码枚举：`card_invalid`、`card_used`、`code_invalid`、`code_expired`。
- 注册为维度的低基数字段：`target`、`direction`、`method`、`result`、`user_type`、`entry`、`trigger_source`、`layout`、`ip`、`login_method`、`has_phone`；`from_level` / `to_level` 为 level 维度（低基数，可注册）。`series`、`country_code` 视基数决定是否注册。
