...
>
> 横幅状态取决于两件事：**① 身份（会员 / 免费）；② 该相邻档体验课是否上过（体验课一次性）**。会员切回已到过的档、会员降级均为**直接切换、不走体验课**；升到全新更高档需一次体验课校准——**达标即抬高 maxReached、报告只出一个 `Update my plan`（可当场切，也可退出后回列表直接切回）；未达标则保持 current、按钮变 `View report` 只回看**。
...
flowchart TB
  ...
  UpTrial --> Judge{达标?}
  Judge -->|达标| UpPass[maxReached 抬到 L+1 · 报告 Update my plan<br/>点 Update 切 current；退出则之后走 Switch to L+1 直切]
...
- **免费转会员落位**：订阅成功后 `current = maxReached = 转化时所领学习计划那档`（在此刻建立 `maxReached`，覆盖免费期采样漂移）。
...
- **切回已到过的更高档**（current+1 ≤ maxReached，即之前下探过、现在切回）：主文案 `Ready to go back up?` + 副文案 `Switch your plan to {L+1}.`，按钮 `Switch to {L+1}` → 二次确认（见 4.5）后**直接切换、不上体验课、从该档首个未上正式课续上**，maxReached 不变。
- **升到新档**（current+1 = maxReached+1，从没到过的更高档）：主文案 `Ready for the next level?` + 副文案 `Try a trial lesson — we'll set your plan based on how you do.`，按钮 `Try {L+1}` → 上一节 current+1 体验课判定（报告页文案沿用 V1.3.0）：
  - **达标** → 该档就此**解锁为「已到过」（maxReached 抬到 L+1）**；报告页只出一个主按钮 `Update my plan`（点击即切 current 到 L+1、弹二次确认排课，见 4.5），用户也可**直接退出不切**。
...
浏览 current-1 level 时，其单元列表**顶部**展示橙色降级横幅，主文案统一 `Feeling too hard?`；分会员 / 免费：

##### 会员

- 副文案 `Switch your plan to {L-1}.`，按钮 `Switch to {L-1}` → 二次确认（见 4.5）后**直接切换到 current-1、不上体验课、从该档首个未上正式课续上**；maxReached 不变，之后想回更高档走升级横幅的「切回」。

##### 免费

- 副文案 `Try the {L-1} lesson — see how it fits.`，按钮 `Try {L-1}` → 上一节 current-1（更简单）体验课 → 体验课报告按所上 level **±1** 真实定级 → 学习计划领取页 → Paywall；可**逐档下探直到舒适**。
...
> 会员的**直接切换**（降级 `Switch to {L-1}` / 切回已到过的档 `Switch to {L+1}` / 升级达标 `Update my plan`）在确认前统一弹**二次确认弹窗**，明确告知新 level 的**排课时机**——依据「**本周已完成课程数**」判断本周是否还有解锁额度（每周 3 节，`WEEK_LESSONS=3`）。免费用户走「体验课 → Paywall」链路、无此排课承诺，不弹此窗。
...
- **级别卡（Level tab）**：三态——`You are here`（current）/ `Completed`（已学完并自然进阶过的更低档）/ `Locked`（其余：更高未解锁档，以及定级时跳过、从没学过的更低隔级档）。不引入 `Recommended`；升 / 降引导由 current±1 顶部横幅承担。
...
- **已完成的更低 level（`Completed`）**：该 level 各已学过的 Unit 显示 `View lessons`，可进入**回看已上过的正式课**（纯复习，不重新计费、不改 current / 排课），进度显示已满。（前提：已上过的正式课内容可回看；若某 Unit 属按周迭代尚未产出的内容，则该 Unit 仍锁。）
...
- **内容按周迭代**：正式内容首发仅第一个 Unit，其后每周迭代一个 Unit；尚未产出内容的 Unit 一律锁、不提供 `View lessons`。
...
- 注册为维度的低基数字段：`target`、`direction`、`method`、`result`、`user_type`、`trigger_source`、`layout`、`ip`、`login_method`、`chat_entry_source`；`from_level` / `to_level` 为 level 维度（低基数，可注册）。`series`、`country_code` 视基数决定是否注册。

---

## 附录 · 升降级横幅穷举表（研发验收基准）

> 状态位：会员用 `current` 与 `maxReached`、免费仅用 `current` 指针（详见 §3、4.2 引言）；下表为「浏览某 level 视图时横幅形态 → 按钮 → 去向 → 埋点」的唯一权威口径，4.3 / 4.4 为其展开说明。
...
| M1  | L = current−1（更低档，必 ≤ maxReached）          | 不涉及              | `Feeling too hard?` / `Switch your plan to {L}.`                                                   | `Switch to {L}`                             | 二次确认(排课时机) → 直切 L、**从 L 首个未上正式课续上**                                 | `learning_path_step`{direction=down, method=direct} → 确认后 `level_change_result`{result, direction=down, method=member_direct} |
| M2  | current < L ≤ maxReached（回到已到过的更高档，含刚达标未切的新档） | 不涉及           | `Ready to go back up?` / `Switch your plan to {L}.`                                                | `Switch to {L}`                             | 二次确认 → 直切 L、续上进度；maxReached 不变                                      | `learning_path_step`{direction=up, method=direct} → `level_change_result`{direction=up, method=member_direct}                 |
| M3  | = maxReached+1（全新更高档）· 未上过                | 未上过              | `Ready for the next level?` / `Try a trial lesson — we'll set your plan based on how you do.`       | `Try {L}`                                   | 上体验课 → **达标** → maxReached=L、报告只出 `Update my plan`（点即切 current；退出则之后按 M2 直切）；**未达标** → 保持 current | `learning_path_step`{direction=up, method=trial}；`trial_report` 曝光；达标切换 `level_change_result`{result=success, method=trial} |
| M4  | = maxReached+1 · 已上过·**未达标**              | 已上过·**维持(不够)**   | `You've already tried {L}` / 回看报告                                                                   | `View report` → 只回看                          | 保持 current（想上更高档靠自然进阶）、**永不被拉低**                                    | 完成判定无切换 → `level_change_result`{result=hold, direction=up, method=trial}                                                      |
...
**表 B · 免费**（无正式排课、**无 maxReached**；仅一个 `current` 指针决定预排课预览与体验课路由，转会员领计划时才建立 `current`/`maxReached`，见 F5）


| #   | 浏览的 level             | 该档体验课 | 横幅主 / 副文案                                                           | 按钮                                 | 点击去向                                  | 埋点                                                                                                |
| --- | --------------------- | ----- | ------------------------------------------------------------------- | ---------------------------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------- |
| F0  | = current             | —     | `You are here`（无横幅）                                                 | —                                  | —                                     | `learning_path` 曝光                                                                                |
| F1  | = current+1           | 未上过   | `Ready for the next level?` / `Try the {L} lesson — see how it fits.` | `Try {L}`                          | 体验课 → ±1 定级(移动指针) → 学习计划领取页 → Paywall | `learning_path_step`{direction=up}; `trial_report`; `purchase_result`{trigger_source=level_up}    |
| F2  | = current−1           | 未上过   | `Feeling too hard?` / `Try the {L} lesson — see how it fits.` | `Try {L}`                          | 体验课 → ±1 定级 → 领取页 → Paywall           | `learning_path_step`{direction=down}; `trial_report`; `purchase_result`{trigger_source=downgrade} |
...
| F5  | — 转会员 —               | —     | —                                                                   | —                                  | `current=maxReached=所领计划档`（此刻建立 maxReached） | `purchase_result`{result=success}                                                                 |
