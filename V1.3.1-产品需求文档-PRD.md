...
| Level·Unit 列表（Learning Path） | 升级横幅（current+1 视图） | • 浏览 current+1 level 时，其单元列表**顶部**展示绿色升级横幅：`Ready for the next level?` + `Try {L+1}`<br>• 该 level 自身各单元均 `Locked`（会员）/ `Go Premium`（免费）；横幅是唯一升级入口 | • 出现场景：所选 level == current+1<br>• **免费**点击 → 定级试课（上 current+1 体验课 → ±1 真实定级 → 学习计划 → Paywall，`trigger_source=level_up`）<br>• **会员**点击 → 升要证明（上 current+1 体验课 → 达标=升 current+1 / 未达标=保持，**永不降级**，`Level up my plan` 手动确认）<br>• **current+1 体验课已消耗** → 横幅变 `View report`（回看=current）<br>• 一次一档：升级只判断 current→current+1 | 接口：当前定级 / 排课更新（沿用 V1.3.0 + 本期扩展） | P0 |
| Level·Unit 列表（Learning Path） | 降级横幅（current-1 视图） | • 浏览 current-1 level 时，其单元列表**顶部**展示橙色降级横幅：`Feeling too hard?` + 会员 `Switch to {L-1}` / 免费 `Try {L-1}`<br>• 与升级横幅同为横幅式，交互一致，只是位于相邻更低 level | • 出现场景：所选 level == current-1 **且 current == 定级起点**（未自然进阶）；自然进阶后 current-1 已是 Completed 级（改走卡片 `View report`），横幅退场<br>• **会员**点击 → 二次确认后**直接切排课到 current-1**（不上体验课、不重新定级）+ toast，当前 Level 与定级起点同步下移<br>• **免费**点击 → 上 current-1 体验课 → 右档报告 → 领取页 → Paywall（`trigger_source=downgrade`）；**免费还需该更低档体验课未消耗**，已消耗则横幅退场、该档改走卡片 `View report`<br>• 一次一档：降级只到 current-1，跨多档逐档 step | 接口：当前定级 / 排课更新 | P0 |
