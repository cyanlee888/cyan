...
| --- | --- | --- | --- | --- | --- |
| Paywall（竖屏改版） | 竖屏单栏支付页 | • 顶部：`Back`（关闭）+ `Restore`<br>• Hero 卡（置于**顶部**）：Dino 形象置于卡片**左上角**，右侧为 `Dino Premium` 徽标 + 标题（**去掉多余副文案**）；其下为**整卡全宽的精简权益清单**（带勾、单列、每条一句）：All 864 core lessons unlocked / Personalized learning path · 3 lessons/week / Report after every lesson / All extension resources unlocked / Full rewards & gamified learning / Chat freely with Dino<br>• **信任背书条**（置于 Hero 卡**下方**、商品卡上方，强视觉）：`13 Years of Proven Trust` + `Backed by over a decade of refinement and trusted by 2M families worldwide.`<br>• 三档商品卡**竖向堆叠排列**（上→下 Annual / Monthly / Weekly）：默认选中 Annual（选中态高亮）；每张卡展示——计划名、**划掉的原价 + 真实价格**（含计费周期，字号较小弱化；划线原价由**后台配置**，默认公式 = 真实价格 + X）、折算后的**天价 `/day`**（字号放大作为主视觉）；年卡（Annual）顶部加「最佳推荐」角标<br>• 底部吸顶 `Subscribe now`（带前进箭头 →）<br>• 协议行：`Cancel anytime · Terms · Privacy`<br>• 异常态：无网络 `Check your connection and try again` | • 出现场景：任意 `trigger_source` 拉起 Paywall<br>• 进入即以竖屏呈现；横屏来源（会员中心 / 锁课）先切竖屏，关闭 / 完成后回来源页（横屏来源回横屏）<br>• 点商品卡 → 切换选中态（边框 + 底色高亮）<br>• `Subscribe now` → 走 V1.3.0 支付链路；成功回来源页 + toast；支付取消静默返回不报错；校验失败留页提示<br>• `Restore` → 恢复购买<br>• 边界：**强制竖屏**；商品 / 计费 / `trigger_source` 归因沿用 V1.3.0，仅版式变化 | 接口：商品 / 订阅 / Restore（沿用 V1.3.0）；`trigger_source` 归因沿用 | P0 |
...
| Level·Unit 列表（Learning Path） | 降级横幅（current-1 视图） | • 浏览 current-1 level 时，其单元列表**顶部**展示橙色降级横幅；与升级横幅同为横幅式、交互一致，只是位于相邻更低 level<br>• 主文案 `Feeling too hard?` + 副文案 会员 `Switch … plan down to {L-1} right away — no trial needed.` / 免费 `Take one {L-1} trial to see if it fits better.`；按钮 会员 `Switch to {L-1}` / 免费 `Try {L-1}`<br>• **体验课已上过**：**会员**降级为直接切档、**不上体验课**，无「已消耗」态（横幅按钮恒为 `Switch to {L-1}`）；**免费**若该更低档体验课**已消耗 → 降级横幅退场（不再展示）**，回看改由该 level 课程卡片的 `View report` 承担（不可重上重定级） | • 出现场景：所选 level == current-1 **且 current == 定级起点**（未自然进阶）；自然进阶后 current-1 已是 Completed 级（改走卡片 `View report`），横幅退场<br>• **会员**点击 → 二次确认后**直接切排课到 current-1**（不上体验课、不重新定级）+ toast，当前 Level 与定级起点同步下移<br>• **免费**点击 → 上 current-1 体验课 → 右档报告 → 领取页 → Paywall（`trigger_source=downgrade`）；**免费还需该更低档体验课未消耗**，已消耗则横幅退场、该档改走卡片 `View report`<br>• 一次一档：降级只到 current-1，跨多档逐档 step | 接口：当前定级 / 排课更新 | P0 |
| Level·Unit 列表（Learning Path） | 跨 level 课程 Lesson CTA | • Lesson 卡片只表达「当前 level 学习 / 复习 / 只读浏览 / 锁定」四态，**不承载任何跨 level 的 `Try` / `Switch`**（升 / 降走相邻 level 顶部横幅） | • **免费**：当前 level 所有正式 Lesson（Lesson 1 起）一律 `Go Premium`（免费唯一可上的是独立**体验课**，不在正式课序列内）；其它任意 level 卡片（含 current±1）→ 一律 `Go Premium`<br>• **会员**：已学过低 level（定级起点 ≤ level < current）→ `View report`（只读，不可重上）；越界档（current+1、被跳过更低档、>current+1）→ `Locked`，点击提示「用该 level 顶部横幅切换」<br>• **体验课一次性**：任一档体验课**已消耗**后，其升 / 降横幅入口替换为 `View report`（回看=current 报告；免费 → 学习计划 → Paywall / 会员 → Continue），不可重上重定级<br>• 所有 level 的 Lesson 内容（words / sentences / objectives）均可**只读浏览** | 接口：当前定级 / 排课更新（沿用 V1.3.0） | P1 |
