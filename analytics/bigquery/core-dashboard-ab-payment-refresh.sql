...
WITH raw_base AS (
  SELECT event_timestamp, event_name, user_pseudo_id, user_id, platform, event_params, user_properties
...
events AS (
  SELECT
    event_timestamp, user_pseudo_id, user_id, platform, event_name,
...
device_groups AS (
  SELECT
    ...
    ARRAY_AGG(channel ORDER BY event_timestamp LIMIT 1)[OFFSET(0)] AS experiment_group,
...
uid_map AS (
  SELECT DISTINCT s.platform, s.experiment_group, e.user_id
  FROM stable s
  JOIN events e
...
ORDER BY platform, experiment_group;
